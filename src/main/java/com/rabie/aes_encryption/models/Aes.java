package com.rabie.aes_encryption.models;
...
public class Aes {
    ...
    // nombre de caractères (32 bits)
    private static int Nb = 4;
    private int Nk;

    // nombre de tours pour l'AES actuel
    private int Nr;
    ...
    // Vecteur d'initialisation (uniquement pour CBC)
    private byte[] iv;

    // matrice nécessaire pour AES (sBox + inversé & rCon)
    ...
    private void init(byte[] key, byte[] iv) {
        ...
        switch (key.length) {
            ...
            case 32:
                ...
                break;
            default:
                throw new IllegalArgumentException("Il ne prend en charge que les clés 128, 192 et 256 bits !");
        }

        // La création de la matrice de stockage pour les states.
        // Seuls 2 states avec 4 lignes et Nb colonnes sont nécessaires.
        state = new int[2][4][Nb];

        // Le vecteur de stockage pour l'expansion de la création de clé.
        w = new int[Nb * (Nr + 1)];
   ...
   // Les 128 bits d'un state sont un décalage XOR qui leur est appliqué avec les 128 bits de la clé dépensés.
    // Round:Un tour de clé w à ajouter.
    // s: renvoie l'addition de la clé par tour
    ...
    // Principales méthodes d'assistance de chiffrement/déchiffrement (pour l'entrée de texte brut/chiffré 128 bits et la sortie de texte chiffré/brut 128 bits) produites par l'algorithme de chiffrement.
    ...
    //Méthodes générales de l'algorithme
